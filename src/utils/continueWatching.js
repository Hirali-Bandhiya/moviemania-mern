...
export const getContinueWatching = () => {
  const list = localStorage.getItem(STORAGE_KEYS.CONTINUE_WATCHING);
  ...
  if (parsed.length > 0 && (parsed[0].id || parsed[0]._id)) {
    ...
    localStorage.setItem(STORAGE_KEYS.CONTINUE_WATCHING, JSON.stringify(migrated));
    return migrated;
  }
  return parsed;
};
...
export const updateContinueWatching = (movieId, progress = 0) => {
  ...
  localStorage.setItem(STORAGE_KEYS.CONTINUE_WATCHING, JSON.stringify(limited));
};
